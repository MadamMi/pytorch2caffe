...
def _view(input, *args):
    x=input.raw_view(input.size(0), *args)
    print(x)
    if not NET_INITTED:
        return x
...
def _permute(input, *args):
    ...
    print(top_blobs)
    layer = caffe_net.Layer_param(name=layer_name,type='Permute',
                                  bottom=[log.blobs(input)],top=top_blobs)

    dims=list(args)
    layer.param.permute_param.order[:]=dims
    log.cnet.add_layer(layer)
    return x
    #dims[0]=0
...
def _contiguous(input,*args):
    ...
    layer.param.flatten_param.axis = 1
    log.cnet.add_layer(layer)
    return x
