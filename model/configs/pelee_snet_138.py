...
class PeleeNet(nn.Module):
    ...
    def forward(self, x):
        ...
        print("forward feature size: ", features.size, features.size(2), features.size(3))
        out = F.avg_pool2d(features, kernel_size=(features.size(2), features.size(3)), stride=1).view(features.size(0), -1)
        if self.drop_rate > 0:
...
def load_model(pretrained_model_path, model_classes=1000, data_classes=365):
    model = PeleeNet(num_classes=model_classes)
    model_dict = model.state_dict()

    pretrained_model = torch.load(pretrained_model_path, map_location='cpu')['state_dict']
