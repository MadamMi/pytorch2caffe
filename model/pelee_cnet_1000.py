...
if __name__ == '__main__':
    ...
    state_dict = torch.load("/mnt/sfshare/test_11111/PeleeNet/weights/peleenet_acc7208.pth.tar",
                            map_location='cpu')['state_dict']
    pretrained_dict = {k.strip('module.'): v for k, v in state_dict.items()
                       if k.strip('module.') in model_dict}
    model_dict.update(pretrained_dict)
    model.load_state_dict(model_dict)
    torch.save(model.state_dict(), '/mnt/sfshare/test_11111/PeleeNet/weights/pelee.pth')
    print(model)
"""